...
export default function AIAvatarPresenter({
  ...
  if (compact) {
    return (
      <div className="relative w-full h-full p-1 bg-black/90 rounded-2xl flex flex-col items-center justify-center border border-indigo-500/40 shadow-xl group">
        <span
          className={`absolute top-1.5 right-1.5 w-2.5 h-2.5 rounded-full z-10 ${
            isPlaying ? 'bg-emerald-400 animate-ping' : 'bg-slate-500'
          }`}
          title={isPlaying ? 'Speaking' : 'Ready'}
        />
        {renderSVG()}
      </div>
    );
